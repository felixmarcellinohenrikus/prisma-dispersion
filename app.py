...
def create_prisma_diagram_matplotlib():
    ...
    # ========================================
    # PRISMA GEOMETRY (segitiga sama kaki)
    # ========================================
    prism_height = 4.0
    ...
    # ========================================
    # ENTRY POINT (di sisi kiri prisma)
    # ========================================
    # Koordinat entry: 35% dari base ke apex di sisi kiri
    t_entry = 0.35
    entry_x = base_left_x + t_entry * (apex_x - base_left_x)
    entry_y = base_y + t_entry * (apex_y - base_y)
    
    # ========================================
    # INCIDENT RAY (dari kiri menuju entry point)
    # ========================================
    # Sudut datang i1 = 45° terhadap normal
    # Normal di sisi kiri mengarah ke kiri-atas
    incident_angle = np.radians(45)
    
    # Arah sinar datang (dari kiri bawah ke kanan atas)
    ...
    # ========================================
    # Sudut bias r1 (lebih kecil dari i1 karena n_prism > n_air)
    r1_angle = np.radians(28)  # contoh untuk n ≈ 1.5
    
    # EXIT POINT (di sisi kanan prisma)
    # Koordinat exit: 50% dari apex ke base di sisi kanan
    t_exit = 0.50
    exit_x = apex_x + t_exit * (base_right_x - apex_x)
    exit_y = apex_y - t_exit * (apex_y - base_y)
    
    # Gambar sinar dalam prisma
    ax.plot([entry_x, exit_x], [entry_y, exit_y], 
            color='#1E3A8A', linewidth=2, alpha=0.8, zorder=4, label='Sinar dalam Prisma')
    
    # ========================================
    # EMERGENT RAYS (spectrum)
    # ========================================
    ...
    for i, (color, label) in enumerate(zip(spectrum_colors, spectrum_labels)):
        # Setiap warna punya sudut bias berbeda (dispersi)
        emergent_angle = np.radians(45 + i * 3)  # sudut berbeda untuk setiap warna
        ...
        # Perpanjangan ke belakang (untuk deviasi)
    ...
    # ========================================
    # NORMAL LINES (garis putus-putus)
    # ========================================
    normal_len = 1.0
    
    # Normal di entry point (sisi kiri)
    # Normal tegak lurus sisi kiri prisma
    ax.plot([entry_x - 0.8, entry_x + 0.8], 
            [entry_y - 0.3, entry_y + 0.3], 
            'k--', linewidth=1, alpha=0.6, zorder=2)
    
    # Normal di exit point (sisi kanan)
    ax.plot([exit_x - 0.5, exit_x + 0.5], 
            [exit_y - 0.4, exit_y + 0.4], 
            'k--', linewidth=1, alpha=0.6, zorder=2)
    ...
    ax.text(0, apex_y - 0.5, f'A = 60°', fontsize=11, fontweight='bold', 
            ha='center', zorder=6)
    ...
    ax.plot(dev_center_x + 2.5*np.cos(dev_arc), 
            dev_center_y + 2.5*np.sin(dev_arc), 
            'r--', linewidth=1.5, zorder=6)
    
    # ========================================
    # COORDINATE LABELS (opsional untuk debugging)
    # ========================================
    # ax.text(entry_x, entry_y, f'({entry_x:.1f}, {entry_y:.1f})', 
    #         fontsize=8, color='blue', ha='center', va='bottom')
    # ax.text(exit_x, exit_y, f'({exit_x:.1f}, {exit_y:.1f})', 
    #         fontsize=8, color='blue', ha='center', va='bottom')
